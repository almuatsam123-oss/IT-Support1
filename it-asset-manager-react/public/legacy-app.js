...
function loadStoredValue(key, fallback = null) {
  if (window.amjaadStorageService) {
    return window.amjaadStorageService.loadValue(key, fallback);
  }
  return fallback;
}
...
function saveStoredValue(key, value) {
  if (window.amjaadStorageService) {
    window.amjaadStorageService.saveValue(key, value);
  }
}

function removeStoredValue(key) {
  if (window.amjaadStorageService) {
    window.amjaadStorageService.remove(key);
  }
}
...
const PAGE_PATHS = {
  ...
  submitRequestPage: "/submit-request",
  trackRequestPage: "/track-request",
  issueHistoryPage: "/issue-history",
  auditLogPage: "/audit-log"
};
const PATH_PAGES = Object.fromEntries(Object.entries(PAGE_PATHS).map(([page, path]) => [path, page]));
Object.assign(PATH_PAGES, {
  "/": "dashboardPage",
  ...
  "/serviceDeskPage": "serviceDeskPage",
  ...
  "/trackRequestPage": "trackRequestPage",
  "/issueHistoryPage": "issueHistoryPage",
...
$("#languageSelect").addEventListener("change", (event) => {
  ...
  saveStoredValue(STORAGE_KEYS.language, currentLanguage);
  applyLanguage();
  setupCategoryOptions();
  renderAll();
  showPage(activePage);
...
window.addEventListener("popstate", () => {
  const requestedPage = getRequestedPage();
  if (requestedPage !== activePage) {
    showPage(requestedPage, { replace: true });
  }
});
...
function setupRequestDraft() {
  ...
  const draft = loadStoredValue(STORAGE_KEYS.requestDraft, null);
  if (draft && confirm(t("Restore unsaved draft?"))) {
    ...
    removeStoredValue(STORAGE_KEYS.requestDraft);
  ...
  form.addEventListener("input", () => {
    ...
    saveStoredValue(STORAGE_KEYS.requestDraft, data);
  });
}
...
function showPage(pageId, options = {}) {
  ...
  const targetPath = PAGE_PATHS[pageId] || "/dashboard";
  if ((location.pathname !== targetPath || location.hash) && !options.skipHistory) {
    if (options.replace) {
...
function generateRequestNumber() {
  ...
  const currentCounter = readNumber(loadStoredValue(STORAGE_KEYS.ticketCounter, 0)) + 1;
  saveStoredValue(STORAGE_KEYS.ticketCounter, currentCounter);
...
function getLoggedInUser() {
  try {
    return loadStoredValue(STORAGE_KEYS.session, null);
  } catch {
    return null;
  }
}
...
function getRequestedPage() {
  ...
  if (document.getElementById(hashPage)?.classList.contains("app-page")) return hashPage;
  const normalizedPath = location.pathname.replace(/\/+$/, "") || "/";
  return PATH_PAGES[normalizedPath] || "dashboardPage";
}
